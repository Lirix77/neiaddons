...
public class CrucibleHandler implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (fluid != null && fluidVolume > 0) currenttip.add(
                I18n.format(
                        "bdew.exnihilo.crucible.fluid",
                        fluid.getLocalizedName(new FluidStack(fluid, 1)),
                        dec.format(fluidVolume)));

        if (!content.isEmpty() && solidVolume > 0) {
            ItemStack stack = new ItemStack((Block) Block.blockRegistry.getObject(content), 1, contentMeta);
            currenttip
                    .add(I18n.format("bdew.exnihilo.crucible.solid", stack.getDisplayName(), dec.format(solidVolume)));
        }
